...
interface LandlordApplicationListProps {
	token?: string;
}
...
export function LandlordApplicationList({ token }: LandlordApplicationListProps) {
			...
			const result = await getLandlordPendingApplications(
				{
					page,
					limit: 10,
					status: 'accepted', // Chỉ lấy các đơn đã được tenant approve
				},
				token,
			);
	...
	const handleSubmit = async () => {
		...
		try {
			const result =
				actionType === 'approve'
					? await landlordApproveApplication(selectedApplication.id, response, token)
					: await landlordRejectApplication(selectedApplication.id, response, token);

			if (result.success) {
				toast.success(
					actionType === 'approve'
						? 'Đã phê duyệt đơn ứng tuyển'
		...
		return (
			<Card>
				<CardContent className="flex flex-col items-center justify-center py-8">
					...
					<p className="text-muted-foreground">Không có đơn ứng tuyển nào cần duyệt</p>
				</CardContent>
			</Card>
		);
	}
		...
		<div className="space-y-4">
					...
					<CardContent className="space-y-2">
						...
						{application.tenantResponse && (
							<div className="rounded-md bg-muted p-3">
								<span className="font-medium">Tenant đã phê duyệt:</span>
								<p className="mt-1 text-sm text-muted-foreground">
									{application.tenantResponse}
								</p>
							</div>
			...
			<Dialog open={!!selectedApplication} onOpenChange={handleCloseDialog}>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>
							{actionType === 'approve' ? 'Phê duyệt' : 'Từ chối'} đơn ứng tuyển
						</DialogTitle>
						<DialogDescription>
							{actionType === 'approve'
					...
					<div className="space-y-4">
						<div>
							...
							<Textarea
								...
								placeholder={
									actionType === 'approve'
										? 'Ví dụ: Ứng viên đáp ứng đầy đủ yêu cầu...'
										: 'Ví dụ: Ứng viên không phù hợp với tiêu chí của tòa nhà...'
								}
								rows={4}
							/>
						</div>
